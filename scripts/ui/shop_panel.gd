...
func _max_quantity(kind: int) -> int:
    return int(_snapshot.get("money", 0)) / GameRules.seed_price(kind)
...
func _update_rows() -> void:
    ...
    for kind in range(GameRules.CropKind.size()):
        ...
        var each := row.get_node("Each") as Label
        var quantity := row.get_node("Quantity") as Button
        var max_button := row.get_node("Max") as Button
        var total_caption := row.get_node("MetaCaption") as Label
        var total := row.get_node("MetaValue") as Label
        row_name.text = GameRules.crop_display_name(kind)
        price.text = "%d" % GameRules.seed_price(kind)
        each.text = "each"
        quantity.text = "×%d" % (_quantity if selected else 0)
        max_button.text = "M\nMAX"
        total_caption.text = "TOTAL" if selected else "STOCK"
        total.text = "%d" % (GameRules.seed_price(kind) * _quantity) if selected else "%d held" % held
        ...
        UiStyle.text(each, 8, UiStyle.MUTED, 600)
        UiStyle.text(total_caption, 8, UiStyle.MUTED, 700, true)
        UiStyle.text(total, 13, UiStyle.GREEN if selected else UiStyle.TEXT, 800, true)
        UiStyle.button(quantity, 14, selected)
        quantity.add_theme_font_override("font", UiStyle.font(700, true))
        quantity.visible = true
        quantity.modulate = Color.WHITE if selected else Color(0.58, 0.61, 0.68, 1.0)
...
func _style_tree(node: Node) -> void:
    ...
    (get_node("Frame/Header/Title") as Label).add_theme_font_size_override("font_size", 13)
    UiStyle.text(get_node("Frame/Header/Title") as Label, 13, UiStyle.CREAM, 800)
    UiStyle.text(get_node("Frame/Header/MoneyValue") as Label, 13, UiStyle.GOLD, 800, true)

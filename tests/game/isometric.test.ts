...
test('selects cells consistently at the proof-map edges', () => {
  expect(gridCellAtWorld(gridToWorld({ x: 0.5, y: 0.5 }, metrics), metrics)).toEqual({
    x: 0,
    y: 0,
  });
  expect(gridCellAtWorld(gridToWorld({ x: 11.999999, y: 11.999999 }, metrics), metrics)).toEqual({
    x: 11,
    y: 11,
  });
  expect(gridCellAtWorld(gridToWorld({ x: 0.5, y: 6.5 }, metrics), metrics)).toEqual({
    x: 0,
    y: 6,
  });
  expect(gridCellAtWorld(gridToWorld({ x: 11.5, y: 6.5 }, metrics), metrics)).toEqual({
    x: 11,
    y: 6,
  });
  expect(gridCellAtWorld(gridToWorld({ x: 6.5, y: 0.5 }, metrics), metrics)).toEqual({
    x: 6,
    y: 0,
  });
  expect(gridCellAtWorld(gridToWorld({ x: 6.5, y: 11.5 }, metrics), metrics)).toEqual({
    x: 6,
    y: 11,
  });
});
...
test('uses a one-nanounit epsilon only at cell lookup boundaries', () => {
  expect(gridCellAtWorld(gridToWorld({ x: 0.999999, y: 4.5 }, metrics), metrics)).toEqual({
    x: 0,
    y: 4,
  });
  expect(gridCellAtWorld(gridToWorld({ x: 0.9999999995, y: 4.5 }, metrics), metrics)).toEqual({
    x: 1,
    y: 4,
  });
});
...
test('the Phaser adapter delegates every projection operation', () => {
  ...
  expect(adapter.projectedBounds(96)).toEqual(
    projectedMapBounds({ width: 12, height: 12 }, metrics, 96),
  );

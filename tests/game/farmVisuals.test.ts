...
import { CROP_DEFINITIONS, CROP_KINDS, visualStage } from '../../src/game/core/cropDefinitions';
import { farmStableOrder, farmVisuals } from '../../src/game/core/farmVisuals';

test('keeps untilled soil hidden in sunny weather', () => {
  expect(farmVisuals({ position: { x: 2, y: 7 }, soil: 'untilled', crop: null }, 'sunny')).toEqual({
    soilFrame: null,
    cropFrame: null,
  });
});
...
test('renders rainy empty soil as wet', () => {
  expect(farmVisuals({ position: { x: 2, y: 7 }, soil: 'tilled', crop: null }, 'rainy')).toEqual({
    soilFrame: 1,
    cropFrame: null,
  });
});
...
test('does not confuse slow-crop progress with a sheet frame', () => {
  ...
  ).toEqual({ soilFrame: 0, cropFrame: 10 });
});
...
test('maps a watered sunny tile to deterministic frames', () => {
  ...
  expect(farmVisuals({ position: { x: 2, y: 7 }, soil: 'tilled', crop: null }, 'sunny')).toEqual({
    soilFrame: 0,
    cropFrame: null,
  });
});
...
test('maps farm tiles to stable row-major depth order', () => {
  expect(Array.from({ length: 9 }, (_, index) => farmStableOrder(index))).toEqual([
